...
public class ExcelCellDebug : ExcelCell
{
    ...
    public ExcelCellDebug(string cellReference, Type valueType, object value, string innerText, uint? numberFormat, int? colIndex = null)
    {
    ...
    public ExcelCellDebug(string cellReference, Type valueType, object value, uint? numberFormat)
        : this(cellReference, valueType, value, value?.ToString() ?? string.Empty, numberFormat, ExcelUtils.GetColumnIndexFromCellReference(cellReference)) { }
    ...
    {
        return $"{CellReference} ({ColIndex}): {Value?.ToString() ?? string.Empty} ({Type?.Name ?? string.Empty}), InnerText: {InnerText}, NumberFormat: {NumberFormat}";
    }
}
