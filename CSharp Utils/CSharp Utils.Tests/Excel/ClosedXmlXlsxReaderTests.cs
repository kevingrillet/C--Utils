...
internal class ClosedXmlXlsxReaderTests
{
    ...
    public void MeasureExecutionTime(string filePath, int nbHeaders, int nbRows)
    {
        ...
        var (headers, rows) = ClosedXmlXlsxReader.ReadExcelFile(filePath);
        stopwatch.Stop();
    ...
    {
        var (headers, rows) = ClosedXmlXlsxReader.ReadExcelFile(path);
        var (_, debugRows) = ClosedXmlXlsxReader.ReadExcelFile(path, true);
