...
namespace CSharp_Utils.Tests.Experiments
{
    ...
    internal class SyncfusionDocxComparerTests
    {
        ...
        {
            var doc1 = new FileStream(Path.GetFullPath($@"Ressources/{document1}.docx"), FileMode.Open);
            var word1 = new WordDocument(doc1, FormatType.Docx);
            doc1.Close();
            var doc2 = new FileStream(Path.GetFullPath($@"Ressources/{document2}.docx"), FileMode.Open);
